    ...
    <div className="max-w-7xl mx-auto px-6 pb-32 pt-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-1">
          ...
          <div className="rounded-2xl border border-black/5 dark:border-white/10 bg-black/[0.02] dark:bg-zinc-900 p-6 shadow-sm flex flex-col lg:h-full lg:max-h-[360px]">
            <h3 className="font-bold text-lg mb-5 border-b border-black/5 dark:border-white/10 pb-3 flex items-center gap-2 shrink-0">
            ...
            <div className="flex flex-col gap-5 lg:flex-1 lg:min-h-0 lg:overflow-y-auto lg:pr-2 lg:[mask-image:linear-gradient(to_bottom,black_82%,transparent_100%)]">
              {recentPosts.map((post) => (
                ...
                >
                  ...
                  <h4 className="text-sm font-bold mb-1 group-hover:text-amber-500 transition-colors line-clamp-2 leading-snug">
                    {post.title}
                  </h4>
            ...
            {series.map((s) => {
              ...
              return (
                <Link key={s} href={`/${s}`} className="group flex flex-col rounded-3xl border border-black/10 dark:border-white/10 bg-[#0a0a0a] overflow-hidden shadow-2xl transition-transform hover:-translate-y-1.5 duration-500">
                   {/* Image - wide rectangle, badge + tags overlaid */}
                   <div className="relative aspect-[16/9] w-full overflow-hidden bg-[#0a0a0a]">
                     ...
                     {seriesTags.length > 0 && (
                       <div className="absolute inset-x-4 bottom-3 flex flex-wrap gap-1.5">
                         {seriesTags.slice(0, 4).map((tag) => (
                           <span
                             key={tag}
                             className="px-2.5 py-1 rounded-full bg-white/15 backdrop-blur-md border border-white/20 text-white text-[9px] font-bold uppercase tracking-wider"
                           >
                             {formatTagLabel(tag)}
                           </span>
                         ))}
                   ...
                   {/* Body - title + description only */}
                   <div className="flex flex-col p-5">
                     <h3 className="text-lg md:text-xl font-black text-white mb-1.5 tracking-tight leading-tight group-hover:text-amber-500 transition-colors">
                       {getSeriesTitle(s)}
                     </h3>
