...
export default function GdCostSurface3d() {
  ...
  useEffect(() => {
    if (!isDesktop) return;
    fetch('/data/ch7-cost-surface.json')
      .then((res) => res.json())
      .then(setData);
  }, [isDesktop]);
  ...
  const zLift = data.costStar * 0.02;

  const path = {
    x: data.trace.map((t) => t.m),
    y: data.trace.map((t) => t.b),
    z: data.trace.map((t) => t.cost + zLift),
    mode: 'lines+markers' as const,
    type: 'scatter3d' as const,
    name: 'Batch GD path',
    line: { color: '#f58231', width: 4 },
    marker: { size: 2.5, color: '#f58231' },
  };

  const startPoint = {
    x: [data.trace[0].m],
    y: [data.trace[0].b],
    z: [data.trace[0].cost + zLift],
  ...
  return (
    <div className="not-prose my-8">
      <div className="rounded-2xl border border-black/5 dark:border-white/5 bg-white dark:bg-black/20 overflow-hidden">
        <Plot
          data={[surface, path, startPoint, minPoint]}
          layout={{
            autosize: true,
            height: 550,
