...
The closed-form solution here only exists because the cost function is a smooth bowl with exactly one minimum, a special case of simple linear regression. Most models don't get that luxury: their cost surfaces don't have a clean algebraic solution, so the minimum has to be found by walking downhill instead, gradient descent. That's the next post.
