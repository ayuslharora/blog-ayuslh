...
[Ch.20](/machine-learning-algorithms/ch20-logistic-regression-perceptron-trick) worked out the Perceptron Trick's logic: represent a line as a weight vector, loop over random points, nudge the line toward whichever one it's currently getting wrong. This post codes that algorithm ([source notebook](https://github.com/campusx-official/100-days-of-machine-learning/blob/main/day58-logistic-regression/perceptron-trick.ipynb)) and then uses the finished implementation to expose exactly why real Logistic Regression needs to be a different, more careful algorithm.
...
The gap between the two lines grows the more separable the data is. `make_classification`'s `class_sep` argument controls how far apart the two clusters sit; pushing it from `10` up to `20` or `30` pulls the clusters apart, and the perceptron line stays glued to whichever cluster it last corrected while the Logistic Regression line keeps re-centering itself in the widening gap.
