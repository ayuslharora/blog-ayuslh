...
Every hyperparameter in the rest of this post is a live control in the explorer below, running a `DecisionTreeClassifier` on exactly that dataset and re-fitting on each change. Push `max_depth` up until the boundary starts carving islands, then bring `min_samples_leaf` up to smooth them back out, and watch the train/test accuracy gap.
