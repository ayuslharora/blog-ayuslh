...
The same shape holds with more than two classes, just harder to draw as a line. With three classes, $p_3 = 1-p_1-p_2$, so the space of valid splits is a triangle instead of a line segment, and entropy becomes a surface over it:

<EntropySimplex3d />

Every corner of the triangle is a pure node, one class at 100%, entropy `0`, exactly like the two ends of the binary curve. The center, all three classes equally likely, is the single highest point, $\log_2 3 \approx 1.585$, the direct three-class analogue of the binary peak at `p = 0.5`. A tree still wants every split to push nodes away from that center and toward a corner; there's just more room to do it in.
...
That similarity isn't a guarantee, though. The two criteria can disagree on some datasets: entropy occasionally produces a more balanced tree, while Gini's speed can come at the cost of a bit more overfitting on specific data. Neither wins outright, which is why `scikit-learn`'s `DecisionTreeClassifier` exposes `criterion` as a hyperparameter (`'gini'` by default, `'entropy'` available), worth including in a grid search alongside the tree's other hyperparameters rather than assumed away.
...
Section 4 (of the [previous post](/machine-learning-algorithms/ch37-decision-tree-intuition)) left one question open: for a numeric feature, which threshold do you actually split on? The answer is a search, not a formula: sort the feature's unique values, take a *candidate* threshold from each one, and compute information gain for that candidate exactly like a categorical split, `< threshold` versus `≥ threshold` playing the role of the two branches. (The video walks this sweep using each observed value directly as a candidate threshold; the more common convention, used here, is the midpoint between each consecutive pair of sorted values, which lands the boundary evenly between two real points rather than on top of one.)
