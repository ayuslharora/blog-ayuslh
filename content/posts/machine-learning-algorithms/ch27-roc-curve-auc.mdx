...
Adding threshold as a third axis makes the sweep explicit: the curve is one continuous path from threshold `1` (bottom, model calls nothing positive, so it sits at the origin) up to threshold `0` (top, model calls everything positive, so it sits at `(1, 1)`). The flat 2D ROC curve is what that path looks like from directly overhead, with the threshold information flattened out of view.
