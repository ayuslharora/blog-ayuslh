...
<img src="/images/ch32-svm-margin-comparison.png" alt="Scatter plot of two linearly separable classes, red x's in the upper left and green plus signs in the lower right. Two parallel diagonal lines both correctly separate the classes: a black line pi_1 sitting roughly centered between the two clusters with room on both sides, and a blue line pi_2 sitting right up against the red cluster with almost no space on that side" className="w-full h-auto" />

Both lines get 100% training accuracy, so accuracy alone can't say which is better. But $\pi_2$ hugs the red cluster closely, leaving almost no breathing room on that side, while $\pi_1$ sits with a comfortable gap from both classes. Intuitively, $\pi_1$ is the safer choice: a new point that lands slightly off from where its class usually sits is far more likely to cross $\pi_2$ than to cross $\pi_1$. That gap is called the **margin**, and SVM's entire strategy is built around maximizing it, which is why it would pick $\pi_1$ here.
...
Lifting the score $w^Tx + b$ into a third axis makes the construction from Section 3 literal: it's a tilted plane, the boundary is its intersection with the flat $\text{score}=0$ sheet, and the two "slide until you touch a point" hyperplanes are the sheets at $\text{score}=+1$ and $-1$. The support vectors are precisely the points touching those sheets, and maximizing the margin is tilting the plane so the two sheets sit as far apart as they can while still keeping every point on the correct side.
