...
The F1 sheet touches the flat arithmetic-mean plane only along the diagonal where precision equals recall. Step off that diagonal in either direction and F1 dips below the plane, always toward the smaller coordinate. Model B lives off the diagonal, which is why its F1 lands under its arithmetic average.
...
As a worked illustration, take a 3-class dog/cat/rabbit classifier evaluated on 100 labeled photos, where rabbits are rare in the dataset (only 5 out of 100). The video uses a roughly balanced version of this matrix; this one deliberately makes rabbit rare and badly predicted, to show what the weighted average can bury.
