...
- You only need to serialize and deserialize data (JSON, XML, YAML, etc.). That is, you save an object whole and later load the same object back by its id, and you never ask the database to filter, join, or aggregate its contents. When that is all you need, a relational schema is overhead, so a document or key-value store is a better fit.
