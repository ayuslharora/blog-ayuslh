---
title: "Ch.15: Feature Scaling: Standardization (Z-Score Normalization)"
...
<MdxImage src="/images/ch15-distance-term-dominance.png" alt="Bar chart on a log scale comparing the squared Age term (400) against the squared Salary term (3.6 billion)" className="w-full h-auto" />
...
**Standardization** (also called Z-score Normalization) transforms a feature column such that its resulting distribution has a **Mean (μ) of 0** and a **Standard Deviation (σ) of 1**.
...
* `μ`: The mean of the feature column across all sample rows.
* `σ`: The standard deviation of the feature column.

### Mathematical Proof of Mean and Standard Deviation

When this transformation is applied to an entire feature column:
1. **New Mean (μ)**: Equal to exactly 0.
2. **New Standard Deviation (σ)**: Equal to exactly 1.
...
1. **Mean Centering**: Subtracting the mean μ shifts the center of gravity of the data points from its original location to the coordinate origin (0, 0).
2. **Scaling by Standard Deviation**: Dividing by σ rescales the spread along each axis. If the original standard deviation was large (for example, σ = 34,000 for salary), dividing by σ compresses the points along that axis. If the original standard deviation was smaller than 1, dividing expands the points.
...
Notice the pattern:
* `scaler.fit(X_train)` learns μ and σ from the training data.
* `scaler.transform(X_train)` scales training data.
* `scaler.transform(X_test)` scales test data using training parameters (μ and σ). Never call `scaler.fit()` on `X_test`!
...
<MdxImage src="/images/ch15-scatter-before-after.png" alt="Side-by-side scatter plots of Age versus Salary before and after standardization, showing the identical point layout on different axis scales" className="w-full h-auto" />
...
<MdxImage src="/images/ch15-kde-before-after.png" alt="KDE density plots of Age and Salary before scaling, where Age is flattened by Salary's scale, and after scaling, where both distributions share a common range" className="w-full h-auto" />
...
<MdxImage src="/images/ch15-accuracy-comparison.png" alt="Grouped bar chart comparing Logistic Regression and Decision Tree accuracy with and without standardization" className="w-full h-auto" />
...
<MdxImage src="/images/ch15-outlier-standardization.png" alt="Scatter plots showing a salary outlier at $350,000 before scaling and its still-extreme scaled position after standardization" className="w-full h-auto" />

Because mean μ and standard deviation σ are themselves highly sensitive to outliers, extreme values distort both μ and σ. If your dataset contains prominent outliers, consider using `RobustScaler` (which uses Median and Interquartile Range) or handle outliers prior to scaling.
