...
<img src="/images/ch23-binning-strategy-comparison.png" alt="Three stacked histograms of the same clustered column, each with a different binning strategy's edges overlaid: Equal Width and Quantile both cut an edge through the middle cluster, while K-Means places both edges cleanly in the gaps between clusters" className="w-full h-auto rounded-lg my-6" />
