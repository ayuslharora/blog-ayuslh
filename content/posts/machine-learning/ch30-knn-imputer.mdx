...
X_train.corr()       # pairwise-complete, original
X_train_knn.corr()   # after KNN imputation
X_train_mean.corr()  # after mean imputation
```
...
<div className="overflow-x-auto my-8">
  <table className="w-full text-left border-collapse">
    <thead>
      <tr className="border-b border-black/10 dark:border-white/10 text-amber-600 dark:text-amber-400">
        <th className="py-3 px-4 font-bold" colSpan={4}>Original (pairwise-complete)</th>
      </tr>
      <tr className="border-b border-black/10 dark:border-white/10 text-amber-600 dark:text-amber-400">
    ...
    <tbody className="divide-y divide-black/5 dark:divide-white/5">
      ...
      <tr>
        <td className="py-3 px-4 font-semibold">Pclass</td>
        <td className="py-3 px-4">-0.380</td>
        <td className="py-3 px-4">1.000</td>
        <td className="py-3 px-4">-0.559</td>
      </tr>
      <tr>
...
<div className="overflow-x-auto my-8">
  <table className="w-full text-left border-collapse">
    <thead>
      <tr className="border-b border-black/10 dark:border-white/10 text-amber-600 dark:text-amber-400">
        <th className="py-3 px-4 font-bold" colSpan={4}>After KNN Imputation</th>
      </tr>
      <tr className="border-b border-black/10 dark:border-white/10 text-amber-600 dark:text-amber-400">
        <th className="py-3 px-4 font-bold"></th>
        <th className="py-3 px-4 font-bold">Age</th>
...
<div className="overflow-x-auto my-8">
  <table className="w-full text-left border-collapse">
    <thead>
      <tr className="border-b border-black/10 dark:border-white/10 text-amber-600 dark:text-amber-400">
        <th className="py-3 px-4 font-bold" colSpan={4}>After Mean Imputation</th>
      </tr>
      <tr className="border-b border-black/10 dark:border-white/10 text-amber-600 dark:text-amber-400">
        <th className="py-3 px-4 font-bold"></th>
        <th className="py-3 px-4 font-bold">Age</th>
...
Two things worth reading in the full matrix rather than a single extracted cell. First, `Pclass`↔`Fare` is `-0.559` in all three tables, identical, because neither column was ever missing, imputing `Age` cannot touch a relationship that doesn't involve `Age` at all. Second, `Age`↔`Pclass` moves from `-0.380` to `-0.339` under mean imputation, every missing `Age` becomes the exact same number regardless of `Pclass`, which mutes the relationship, and KNN lands closer at `-0.365`, precisely because each fill was chosen *using* that row's own `Pclass` and `Fare`. But `Age`↔`Fare` tells a less clean story: `0.096` originally, KNN moves it to `0.090`, mean to `0.091`, KNN is not the closer of the two on this particular pair, even though `Fare` was one of the two columns it used to pick each fill. Multivariate imputation uses the other columns, it doesn't guarantee every relationship improves, on this split it's a clear win for the relationship the technique was pointed at (`Age`↔`Pclass`) and a wash on the other one (`Age`↔`Fare`).
