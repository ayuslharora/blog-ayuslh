...
`r'(\d+)$'` anchors the digit capture to the **end** of the string, since the number always comes last, whatever precedes it (if anything) is the category. Stripping that trailing number out and trimming whitespace leaves the prefix behind; tickets that were pure numbers end up with an empty prefix, which gets relabeled `'NONE'` rather than left blank.
