...
date: "2026-08-04T07:00:00+05:30"
...
*Inspired by:* [YouTube](https://www.youtube.com/watch?v=9xiX-I5_LQY&t=1s)
...
<MdxImage src="/images/ch24-cabin-deck-distribution.png" alt="Bar chart of passenger counts per deck letter extracted from the Cabin column, showing deck C with the most passengers (59), followed by B, D, E, A, F, G, and T" className="w-full h-auto rounded-lg my-6" />
...
**The fix:** route each row into one of two new columns instead of splitting each cell. Two tools do the work, `pd.to_numeric()` and `np.where()`, so it helps to look at each on its own before combining them.

**Step 1: pull out the numbers, one line at a time.**

```python
df['number_numerical'] = pd.to_numeric(df['number'], errors='coerce')
```

`pd.to_numeric()` tries to convert every value in `number` into an actual number. Normally, if it hits something that isn't a number (like the string `'Alone'`), it would throw an error and stop. `errors='coerce'` tells it to relax instead: whenever a value can't be converted, just put `NaN` there and keep going.

So for a row with `number = 2`, `number_numerical` becomes `2.0`. For a row with `number = 'Alone'`, `number_numerical` becomes `NaN`, since `'Alone'` isn't a number.

**Step 2: pull out the categories, using the gaps `NaN` left behind.**
...
`np.where(condition, value_if_true, value_if_false)` is basically an if/else statement applied to an entire column at once, row by row. Here the three pieces are:

- **condition:** `df['number_numerical'].isnull()`, true wherever Step 1 just filled in a `NaN` (meaning that row's original value wasn't a number, i.e. it was `'Alone'`).
- **value if true:** `df['number']`, the original value. So wherever the condition is true, `number_categorical` copies over whatever was in `number` (which will be `'Alone'`, since that's the only thing that made the condition true).
- **value if false:** `np.nan`. For every row that *was* a real number, `number_categorical` just gets `NaN`, since there's no category to record there.

Putting both steps together and sampling some rows:
...
Row `709` had `number = 2`: it became `2.0` in `number_numerical`, and `NaN` in `number_categorical`, since there was nothing categorical about it. Row `439` had `number = 'Alone'`: the opposite happened, `NaN` in `number_numerical` because `'Alone'` isn't a number, and `'Alone'` itself carried over into `number_categorical`. Every row lands cleanly in exactly one of the two new columns, never both, which is the signature of this type of mixed variable.
...
<MdxImage src="/images/ch24-ticket-category-reduction.png" alt="Two panels: a bar chart showing unique value count dropping from 681 raw tickets to 45 extracted ticket categories, and a horizontal bar chart of the top ticket category prefixes such as PC, C.A., and STON/O 2." className="w-full h-auto rounded-lg my-6" />
