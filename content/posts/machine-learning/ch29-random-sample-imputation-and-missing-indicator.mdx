...
```python
X_train['age'].cov(X_train['family'])
X_train['age_imputed'].cov(X_train['family'])
X_train['age_median'].cov(X_train['family'])
```
...
<div className="overflow-x-auto my-8">
  <table className="w-full text-left border-collapse">
    <thead>
      <tr className="border-b border-black/10 dark:border-white/10 text-amber-600 dark:text-amber-400">
        <th className="py-3 px-4 font-bold">State</th>
        <th className="py-3 px-4 font-bold">Covariance (age, family)</th>
      </tr>
    </thead>
    <tbody className="divide-y divide-black/5 dark:divide-white/5">
      <tr>
...
Both techniques push the covariance toward zero by almost the same amount, roughly a 20% shrink from `-6.50`, random sample landing at `-5.18`, median at `-5.11`. That's a useful, slightly counterintuitive result: even though random sample imputation left `age`'s own distribution and variance almost untouched, it does *not* preserve `age`'s relationship with `family` any better than plain median imputation did. That checks out with how the fill values are chosen, each draw is a real value from the `age` column, but drawn independently of whatever `family` happens to be on that same row, so the pairing between the two columns on every imputed row is essentially random. Distribution shape and variance are what this technique is good at protecting; relationships with other columns aren't part of the deal, no univariate technique can fix that, since by definition it never looks at the other columns while filling one of them in.
