...
```python
def random_sample_impute(train_df, test_df, variable, seed=0):
    train_df[variable + '_imputed'] = train_df[variable]
    test_df[variable + '_imputed'] = test_df[variable]

    random_sample_train = train_df[variable].dropna().sample(
        train_df[variable].isnull().sum(), random_state=seed)
    random_sample_train.index = train_df[train_df[variable].isnull()].index
    train_df.loc[train_df[variable].isnull(), variable + '_imputed'] = random_sample_train

    random_sample_test = train_df[variable].dropna().sample(
        test_df[variable].isnull().sum(), random_state=seed, replace=True)
    random_sample_test.index = test_df[test_df[variable].isnull()].index
    test_df.loc[test_df[variable].isnull(), variable + '_imputed'] = random_sample_test

    return train_df, test_df

X_train, X_test = random_sample_impute(X_train, X_test, 'age')
X_train, X_test = random_sample_impute(X_train, X_test, 'fare')
```

Two `.sample()` calls worth walking through carefully. `train_df[variable].dropna().sample(n, random_state=seed)` pulls `n` random values from the column's own non-null entries, where `n` is exactly how many values are missing, one replacement value per hole. The line right after, `random_sample.index = train_df[train_df[variable].isnull()].index`, is what actually places each drawn value into the correct row: it overwrites the sample's index with the index labels of the missing rows, so `.loc[...] = random_sample` lines each drawn value up with a *specific* missing row by index rather than by position. Without that reassignment, pandas would try to align on the original indices and the assignment wouldn't line up at all. The test-set version does the same thing but draws from `train_df`'s values (never `test_df`'s own, that would leak information the model shouldn't have) and adds `replace=True`, since the test set can need more or fewer draws than happen to exist in the training column.
...
### It Still Disturbs Correlation

```python
X_train['age'].corr(X_train['family'])          # -0.299
X_train['age_imputed'].corr(X_train['family'])  # -0.219
```
...
For comparison, median imputation moved this same correlation to `-0.243`, so random sample imputation doesn't come out ahead here, in fact it moves the correlation slightly *more*, since it's introducing genuinely random noise rather than a single constant that at least stays predictable. Distribution shape and variance are what this technique is good at protecting; relationships with other columns aren't part of the deal, no univariate technique can fix that, since by definition it never looks at the other columns.

---

## Trying It on House Prices (Categorical)

The same function works unchanged on categorical data, `.sample()` doesn't care whether the values it's drawing are numbers or strings. This reuses the [categorical imputation post's](/machine-learning/ch28-categorical-imputation) dataset: Ames housing, narrowed to `GarageQual`, `FireplaceQu`, and `SalePrice`, where `FireplaceQu` is missing on 47.7% of rows with no dominant category, exactly the situation mode imputation handled badly.

```python
rs = X_train['FireplaceQu'].dropna().sample(
    X_train['FireplaceQu'].isnull().sum(), random_state=0)
rs.index = X_train[X_train['FireplaceQu'].isnull()].index
X_train['FireplaceQu_imputed'] = X_train['FireplaceQu']
X_train.loc[X_train['FireplaceQu'].isnull(), 'FireplaceQu_imputed'] = rs
...
The fix: don't reseed randomness per request, tie it to something stable about the request itself. A common approach is deriving the seed from the specific observation, so the *same* incoming row always draws the *same* replacement value, while two different incomplete rows can still draw two different values. The randomness stays real across rows, it just stops being random across repeated identical requests.
...
df = pd.read_csv('https://raw.githubusercontent.com/mwaskom/seaborn-data/master/titanic.csv')
df = df[['age', 'embarked', 'sex', 'pclass', 'survived']]
X = df.drop(columns='survived')
y = df['survived']
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=2)
...
`age` (numerical) and `embarked` (categorical) both have missing values, `sex` and `pclass` don't. Two sub-pipelines, one for each type, feed into a `ColumnTransformer`:
...
- **`num_pipe`** is a one-step `Pipeline` wrapping a `SimpleImputer` under the name `'impute'`. It's still a `Pipeline`, not just a bare transformer, specifically so that name can be targeted later.
- **`cat_pipe`** is a two-step `Pipeline`: `'impute'` fills missing categories, then `'encode'` one-hot encodes the result. These run in sequence, imputation has to happen before encoding, since `OneHotEncoder` can't handle `NaN` values on its own. `handle_unknown='ignore'` tells the encoder to zero out any category it never saw during `fit` (relevant once `X_test` gets transformed) instead of raising an error.
- **`trf1`** is a `ColumnTransformer` that routes `['age']` through `num_pipe` and `['embarked', 'sex', 'pclass']` through `cat_pipe`, in parallel, then concatenates the results back into one array. This is the same pattern used for the `age`/`fare` split back in the [mean/median post](/machine-learning/ch27-mean-median-imputation), just with a full pipeline instead of a bare `SimpleImputer` in each slot.
- **`pipe`** chains `trf1` into a `LogisticRegression`, so calling `pipe.fit(X_train, y_train)` runs the entire preprocessing-plus-model sequence in one call.

The `strategy='mean'` and `strategy='most_frequent'` set on the two imputers above are just starting points, values the pipeline needs *something* set to in order to be constructible, not the final answer. That's what `param_grid` is for, it lists the strategies actually worth comparing:

```python
param_grid = {
    'trf1__num__impute__strategy': ['mean', 'median'],
    'trf1__cat__impute__strategy': ['most_frequent', 'constant'],
}
```

Each key is a **path into the nested pipeline**, joined with double underscores, one segment per level of nesting: `trf1` (the step name inside `pipe`) → `num` (the transformer name inside `trf1`) → `impute` (the step name inside `num_pipe`) → `strategy` (the actual constructor argument on that `SimpleImputer`). `GridSearchCV` doesn't know anything about imputers or pipelines specifically, it just calls `.set_params(**{that path: that value})` on a clone of `pipe` for every combination in the grid, and `scikit-learn`'s `.set_params()` is what knows how to walk a dotted (here, double-underscore) path down through nested estimators. Getting a segment wrong, `trf1__num_impute__strategy` instead of `trf1__num__impute__strategy` for example, fails immediately with an "invalid parameter" error rather than silently ignoring it, which is a fast way to double check the path matches the actual step names used above.
...
`cv=5` means every one of the 2×2=4 combinations in the grid gets evaluated with 5-fold cross-validation, the training data is split five ways, each combination trains on four folds and validates on the held-out fifth, five times over, and the five scores get averaged into one number per combination. That's `4 × 5 = 20` total model fits for this grid, `.fit()` runs all of them. `scoring='accuracy'` is what those folds get judged on; by default `GridSearchCV` also refits one final model on the *entire* training set using whichever combination scored best, that refit model is what ends up accessible as `grid.best_estimator_`.

```text
best_params_: {'trf1__cat__impute__strategy': 'constant', 'trf1__num__impute__strategy': 'median'}
best_score_:  0.8033
```

`best_params_` reports back using the exact same double-underscore paths from `param_grid`, and on this split the winner is median for `age`, constant-fill for the categorical columns. `best_score_` is the average cross-validated accuracy for that winning combination, 80.33%, not a test-set score, that number only comes from calling `grid.score(X_test, y_test)` separately. On a small dataset like this the gap between combinations won't always be dramatic, but the value of the approach doesn't depend on the dataset being large: it replaces "which imputation strategy should I use" with "let cross-validation answer that," the same shift `GridSearchCV` already makes for every other hyperparameter in a pipeline.
