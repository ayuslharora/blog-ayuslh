...
*Inspired by:* [YouTube](https://www.youtube.com/watch?v=aUnNWZorGmk)
...
Five columns sit comfortably under 5%: `training_hours` (4.0%), `city_development_index` (2.5%), `education_level` (2.4%), `enrolled_university` (2.0%), and `experience` (0.3%). Those five are what this post applies CCA to, picked out with a list comprehension instead of typing the names by hand, so the same line still works if the dataset's columns change:

```python
cols = [col for col in df.columns if 0 < df[col].isnull().mean() < 0.05]
cols
```
...
<MdxImage src="/images/ch26-before-after-cca.png" alt="Two-panel chart: left panel shows overlapping histograms of training hours before and after Complete Case Analysis, nearly identical in shape; right panel shows a grouped bar chart of education level category proportions before and after CCA, with matching bar heights across all five categories" className="w-full h-auto rounded-lg my-6" />
