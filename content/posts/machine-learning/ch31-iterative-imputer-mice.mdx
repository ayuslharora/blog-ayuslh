...
That tie is worth sitting with rather than explaining away: it does *not* mean `IterativeImputer` filled `Age` the same way mean imputation did.
...
Mean imputation writes `29.79` regardless of class or fare. `IterativeImputer` gives a third-class, low-fare passenger (`77`) a younger predicted age (`24.83`) and a first-class, high-fare passenger (`334`) an older one (`35.00`), because it's actually using the correlation between `Age`, `Pclass`, and `Fare` on every fill. `LogisticRegression` on this particular 3-feature split just happens to draw the same decision boundary either way, a coincidence of this dataset and this split, not something to expect in general.
