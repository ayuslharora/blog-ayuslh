...
```mermaid
flowchart TD
    subgraph ClientHost ["Client OS Kernel (192.168.1.5:8000)"]
        ClientState["<b>Client State Memory Table:</b><br/>• Sequence Tracking (ISN_client)<br/>• Flow Control (Receive Window Size)<br/>• State: CLOSED -> SYN-SENT -> ESTABLISHED"]
    end

    subgraph UntrustedNetwork ["Untrusted IP Network (Layer 3)"]
        Wire["IP Packets carrying TCP Segments<br/>(Can drop, delay, or reorder packets)"]
    end

    subgraph ServerHost ["Server OS Kernel (10.0.0.1:80)"]
        ServerState["<b>Server State Memory Table:</b><br/>• Sequence Tracking (ISN_server)<br/>• Flow Control (Receive Window Size)<br/>• State: LISTEN -> SYN-RCVD -> ESTABLISHED"]
    end

    ClientState <--> Wire <--> ServerState

    style ClientHost fill:#1e3a8a,stroke:#3b82f6,color:#fff
    style UntrustedNetwork fill:#1e293b,stroke:#64748b,color:#fff
