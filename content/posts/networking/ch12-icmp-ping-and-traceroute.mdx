---
title: "Ch.12: ICMP, Ping, and Traceroute"
description: "A comprehensive guide to Internet Control Message Protocol (ICMP), how Ping measures RTT, how Traceroute cleverly uses TTL to discover intermediate routers, and why asterisks (***) appear in traceroute output."
date: "2026-07-22T02:46:07+05:30"
tags: ["networking", "icmp", "ping", "traceroute", "ttl", "ip"]
subtopic: "OSI Model, Routing & Diagnostics"
draft: false
---

*Inspired by:* [YouTube](https://www.youtube.com/watch?v=VzPFY40qAhQ)

In the previous post, we explored routing in detail, examining how data packets jump across switches, local subnets, and internet routers to reach external services. However, when sending data over complex networks, things do not always go smoothly. Packets can get stuck in infinite routing loops, targets might be offline, or targeted ports might not have active services listening.

To diagnose network issues and communicate errors across devices, networks rely on **ICMP (Internet Control Message Protocol)** and its utility tools: **Ping** and **Traceroute**. In this post, we will break down how ICMP operates at Layer 3, examine how Ping and Traceroute function under the hood, analyze Time-To-Live (TTL) mechanics, and explain why `* * *` (star star star) appears in traceroute outputs.

---

## What is ICMP (Internet Control Message Protocol)?

**ICMP (Internet Control Message Protocol)** is a core protocol in the Internet Protocol Suite used by network devices (such as routers, switches, and host machines) to send operational messages, diagnostic status, and error notifications back to the source device.

```mermaid
flowchart TD
    subgraph Layer4 ["Layer 4: Transport Layer (Uses Ports)"]
        TCP["<b>TCP</b><br>Port 80/443"]
        UDP["<b>UDP</b><br>Port 53/5060"]
        QUIC["<b>QUIC</b><br>Port 443"]
    end

    subgraph Layer3 ["Layer 3: Network Layer (IP Addresses Only)"]
        IP["<b>IP Protocol</b><br>Addressing & Routing"]
        ICMP["<b>ICMP Protocol</b><br>Diagnostics & Errors"]
    end

    Layer4 --> Layer3

    style Layer4 fill:#1e1b4b,stroke:#6366f1,stroke-width:2px,color:#fff
    style Layer3 fill:#31103f,stroke:#c084fc,stroke-width:2px,color:#fff
    style TCP fill:#3730a3,stroke:#818cf8,color:#fff
    style UDP fill:#3730a3,stroke:#818cf8,color:#fff
    style QUIC fill:#3730a3,stroke:#818cf8,color:#fff
    style IP fill:#6b21a8,stroke:#d8b4fe,color:#fff
    style ICMP fill:#166534,stroke:#4ade80,color:#fff
```

### Key Characteristics of ICMP

1. **Operates at Layer 3 (Network Layer)**: ICMP sits directly at Layer 3 alongside IP. It does not carry application data or user payloads.
2. **No Port Numbers**: Unlike Layer 4 protocols (TCP and UDP) which use port numbers (such as port 80 for HTTP or port 443 for HTTPS) to direct traffic to specific applications, ICMP does not have ports. It relies purely on IP addresses to deliver diagnostic messages.
3. **Control and Error Reporting**: ICMP notifies the sender when network anomalies occur rather than guaranteeing data delivery.

### Common ICMP Messages and Use Cases

* **Host Unreachable**: Sent by an intermediate router when it cannot locate or route traffic to the requested destination IP address.
* **Port Unreachable**: Sent by the destination host when an IP packet arrives, but no process or application is listening on the targeted destination port.
* **Time Exceeded (TTL Expired)**: Sent by a router when an IP packet's Time-To-Live counter reaches zero before reaching its final destination.

---

## What is Ping and How Does It Work Under the Hood?

**Ping** is a command-line network utility used to test whether a remote server or host machine is reachable over an IP network, while also measuring the **Round Trip Time (RTT)** (latency) for messages sent from the source host to the destination.

### Network Setup & Ping Flow

```mermaid
flowchart LR
    subgraph Client ["Source Host"]
        Laptop["<b>My Machine</b><br>10.0.0.1"]
    end

    subgraph Local ["Local Gateway"]
        HomeRouter["<b>Home Router</b><br>10.0.0.4"]
    end

    subgraph Internet ["INTERNET"]
        R20["<b>Router</b><br>20.0.0.1"]
        R30["<b>Router</b><br>30.0.0.1"]
        R40["<b>Router</b><br>40.0.0.1"]
        R50["<b>Router</b><br>50.0.0.1"]

        R20 --> R30 --> R40 --> R50
    end

    subgraph Target ["Destination Host"]
        Google["<b>google.com</b><br>60.0.0.1"]
    end

    Laptop --> HomeRouter
    HomeRouter --> R20
    R50 --> Google

    style Client fill:#451a03,stroke:#f97316,stroke-width:2px,color:#fff
    style Laptop fill:#78350f,stroke:#fbbf24,color:#fff

    style Local fill:#1e293b,stroke:#64748b,stroke-width:2px,color:#fff
    style HomeRouter fill:#334155,stroke:#94a3b8,color:#fff

    style Internet fill:#3b0764,stroke:#a855f7,stroke-width:2px,color:#fff
    style R20 fill:#581c87,stroke:#c084fc,color:#fff
    style R30 fill:#581c87,stroke:#c084fc,color:#fff
    style R40 fill:#581c87,stroke:#c084fc,color:#fff
    style R50 fill:#581c87,stroke:#c084fc,color:#fff

    style Target fill:#451a03,stroke:#f97316,stroke-width:2px,color:#fff
    style Google fill:#78350f,stroke:#fbbf24,color:#fff
```

### IP Packet Header Structure During Request & Reply

```mermaid
flowchart TD
    subgraph RequestPacket ["Outgoing IP Packet Header (ICMP Echo Request)"]
        direction LR
        SrcReq["<b>10.0.0.1</b><br>Source IP"] --- PayloadReq["<b>ICMP echo request</b><br>Sequence: 1 | TTL: 64"] --- DstReq["<b>60.0.0.1</b><br>Destination IP"]
    end

    subgraph ReplyPacket ["Incoming IP Packet Header (ICMP Echo Reply)"]
        direction LR
        SrcRep["<b>60.0.0.1</b><br>Source IP"] --- PayloadRep["<b>ICMP echo reply</b><br>Sequence: 1 | RTT: 18ms"] --- DstRep["<b>10.0.0.1</b><br>Destination IP"]
    end

    style RequestPacket fill:#18181b,stroke:#3f3f46,stroke-width:2px,color:#fff
    style SrcReq fill:#7f1d1d,stroke:#fca5a5,color:#fff
    style PayloadReq fill:#14532d,stroke:#86efac,color:#fff
    style DstReq fill:#7f1d1d,stroke:#fca5a5,color:#fff

    style ReplyPacket fill:#18181b,stroke:#3f3f46,stroke-width:2px,color:#fff
    style SrcRep fill:#7f1d1d,stroke:#fca5a5,color:#fff
    style PayloadRep fill:#14532d,stroke:#86efac,color:#fff
    style DstRep fill:#7f1d1d,stroke:#fca5a5,color:#fff
```

### Ping Step-by-Step Execution

1. **DNS Lookup**: Ping performs a DNS resolution for `google.com` to discover its IP address (e.g., `60.0.0.1`).
2. **ICMP Echo Request**: The local operating system constructs an **ICMP Echo Request** message. It wraps this inside an IP packet with:
   * **Source IP**: `10.0.0.1` (Laptop IP)
   * **Destination IP**: `60.0.0.1` (Google Server IP)
3. **Network Traversal**: The packet is forwarded across local switches and internet routers until it reaches the destination host.
4. **ICMP Echo Reply**: Upon receiving the Echo Request, the target host generates an **ICMP Echo Reply** packet with:
   * **Source IP**: `60.0.0.1` (Google Server IP)
   * **Destination IP**: `10.0.0.1` (Laptop IP)
5. **RTT Latency Calculation**: When the laptop receives the ICMP Echo Reply, it calculates the time difference between sending the request and receiving the response. This duration is reported in milliseconds (ms) as the Round Trip Time (RTT).

### Handling Host Unreachability

What happens if you attempt to ping a server IP that does not exist or is offline?

```mermaid
flowchart LR
    subgraph Source ["Source Machine"]
        Laptop["<b>My Machine</b><br>10.0.0.1"]
    end

    subgraph ISP ["ISP Network"]
        Router40["<b>ISP Router</b><br>40.0.0.1"]
    end

    subgraph DeadTarget ["Unreachable Host"]
        DeadHost["<b>Dead Host</b><br>60.0.0.1"]
    end

    Laptop -- "1. ICMP Echo Request" --> Router40
    Router40 -.-x|"2. Host Not Found (Dropped)"| DeadHost
    Router40 -- "3. ICMP Error: Host Unreachable" --> Laptop

    style Source fill:#451a03,stroke:#f97316,stroke-width:2px,color:#fff
    style Laptop fill:#78350f,stroke:#fbbf24,color:#fff

    style ISP fill:#1e293b,stroke:#64748b,stroke-width:2px,color:#fff
    style Router40 fill:#334155,stroke:#94a3b8,color:#fff

    style DeadTarget fill:#3f3f46,stroke:#71717a,stroke-width:2px,color:#fff
    style DeadHost fill:#18181b,stroke:#52525b,color:#71717a
```

When an intermediate router (such as `40.0.0.1`) determines that the destination network or host `60.0.0.1` does not exist:
1. The router drops the incoming ICMP Echo Request packet.
2. The router generates an **ICMP Host Unreachable** error message.
3. It sets the **Source IP** to its own IP (`40.0.0.1`) and the **Destination IP** to your laptop (`10.0.0.1`).
4. Upon receiving this ICMP packet, your terminal displays `Destination Host Unreachable`.

---

## Latency, Bandwidth, and Throughput: The Tollbooth Analogy

When Ping reports `RTT: 18ms`, what actually makes up those 18 milliseconds? And is that the same thing as your "100 Mbps" internet plan? These terms get mixed up constantly, so let's separate them with an analogy: cars traveling through a tollbooth onto a highway.

Imagine a tollbooth (a router's outgoing link) feeding cars (packets) onto a highway (the physical cable) leading to a distant city (the destination):

* **Bandwidth**: The maximum rate at which the tollbooth can physically push cars onto the highway, a theoretical ceiling (e.g., "100 Mbps"). It describes capacity, not actual usage.
* **Throughput**: The actual number of cars getting through per second, in practice. If the tollbooth is congested or the highway is under construction, throughput is always less than or equal to bandwidth.
* **Latency**: The total time for a single car to leave the tollbooth and arrive at the destination city, the delay experienced by that one car. This is roughly what Ping's RTT measures (doubled, since it's a round trip).

### Transmission Delay vs. Propagation Delay

Latency itself is made up of a few components, but these two are the most important, and the most commonly confused:

* **Transmission Delay**: The time for the tollbooth to push *all* the cars (bits) of one packet onto the highway. A bigger packet takes longer to fully enter the highway. Formula: `Packet Size / Bandwidth`. This depends on how much data you're pushing out.
* **Propagation Delay**: Once a car is on the highway, how long it takes to physically drive from one end to the other at a roughly fixed speed (close to the speed of light in the cable). Formula: `Distance / Propagation Speed`. This depends purely on physical distance, not how much data you're sending.

A tiny ICMP Echo Request has a negligible transmission delay, but if you're pinging a server on the other side of the world, propagation delay (physical distance) dominates the RTT you see. (Two more delays exist: queuing delay, waiting in line at the tollbooth before your turn, and processing delay, the time a router takes to inspect and decide where to forward a packet, but transmission and propagation are the two intuitive ones to start with.)

---

## Understanding TTL (Time-To-Live) Mechanics

Without safeguards, a misconfigured router routing table could cause an IP packet to loop endlessly between routers, consuming network bandwidth indefinitely.

To prevent infinite loops, every IP packet header includes a field called **TTL (Time-To-Live)**.

### How TTL Functions During Hop Traversal

```mermaid
flowchart LR
    subgraph Client ["Source"]
        Laptop["<b>My Machine</b><br>10.0.0.1"]
    end

    subgraph Gateway ["Local Gateway"]
        HomeRouter["<b>Home router</b><br>10.0.0.4"]
    end

    subgraph Internet ["INTERNET"]
        R20["<b>20.0.0.1</b><br>Hop 1 (TTL=3)"]
        R30["<b>30.0.0.1</b><br>Hop 2 (TTL=2)"]
        R40["<b>40.0.0.1</b><br>Hop 3 (TTL=1 -> 0)"]
        R50["<b>50.0.0.1</b><br>Hop 4"]

        R20 --> R30 --> R40
        R40 -.-x|"Dropped (TTL=0)"| R50
    end

    subgraph Dest ["Target"]
        Google["<b>google.com</b><br>60.0.0.1"]
    end

    Laptop -- "Sent: TTL=4" --> HomeRouter
    HomeRouter --> R20
    R50 --> Google
    R40 -- "ICMP Time Exceeded" --> Laptop

    style Client fill:#451a03,stroke:#f97316,stroke-width:2px,color:#fff
    style Laptop fill:#78350f,stroke:#fbbf24,color:#fff

    style Gateway fill:#1e293b,stroke:#64748b,stroke-width:2px,color:#fff
    style HomeRouter fill:#334155,stroke:#94a3b8,color:#fff

    style Internet fill:#3b0764,stroke:#a855f7,stroke-width:2px,color:#fff
    style R20 fill:#581c87,stroke:#c084fc,color:#fff
    style R30 fill:#581c87,stroke:#c084fc,color:#fff
    style R40 fill:#991b1b,stroke:#f87171,stroke-width:3px,color:#fff
    style R50 fill:#3f3f46,stroke:#71717a,color:#9ca3af

    style Dest fill:#451a03,stroke:#f97316,stroke-width:2px,color:#fff
    style Google fill:#78350f,stroke:#fbbf24,color:#fff
```

### IP Header Packets During TTL Expiration

```mermaid
flowchart TD
    subgraph SentHeader ["Outgoing Request Packet Header (TTL Expired at 40.0.0.1)"]
        direction LR
        SrcS["<b>10.0.0.1</b><br>Source IP"] --- PayS["<b>ICMP echo request, TTL=0</b><br>(TTL decremented to 0 at router)"] --- DstS["<b>60.0.0.1</b><br>Destination IP"]
    end

    subgraph ExceededHeader ["Returned ICMP Error Packet Header"]
        direction LR
        SrcE["<b>40.0.0.1</b><br>Router Source IP"] --- PayE["<b>Time To Live Exceeded (ICMP)</b><br>Type 11 Code 0"] --- DstE["<b>10.0.0.1</b><br>Laptop Destination IP"]
    end

    style SentHeader fill:#18181b,stroke:#3f3f46,stroke-width:2px,color:#fff
    style SrcS fill:#7f1d1d,stroke:#fca5a5,color:#fff
    style PayS fill:#14532d,stroke:#86efac,color:#fff
    style DstS fill:#7f1d1d,stroke:#fca5a5,color:#fff

    style ExceededHeader fill:#18181b,stroke:#3f3f46,stroke-width:2px,color:#fff
    style SrcE fill:#7f1d1d,stroke:#fca5a5,color:#fff
    style PayE fill:#14532d,stroke:#86efac,color:#fff
    style DstE fill:#7f1d1d,stroke:#fca5a5,color:#fff
```

1. **Initial TTL Value**: The originating OS sets an initial TTL value on outgoing IP packets (commonly 64 on macOS/Linux or 128 on Windows).
2. **Hop Decrement**: Every router that forwards the packet decrements the TTL field by exactly 1.
3. **TTL Expiration at Zero**: If a packet's TTL reaches 0 before reaching the destination:
   * The router holding the packet drops it immediately.
   * The router constructs an **ICMP Time Exceeded** (Type 11) error message.
   * The router sets the **Source IP** of the message to its own IP address and sends it back to the originating machine.

### Observing TTL Expiration Manually

You can test TTL behavior by setting a custom maximum TTL of 1 using ping (for example, `ping google.com -m 1` on macOS):

```text
PING google.com (142.250.190.46): 56 data bytes
36 bytes from 192.168.1.1: Time to live exceeded
```

Because TTL was limited to 1, the packet reached your home router (`192.168.1.1`), which decremented TTL from 1 to 0, dropped the packet, and returned an `ICMP Time Exceeded` message back to your computer.

---

## Traceroute: How It Works Under the Hood

**Traceroute** (invoked as `traceroute` on macOS/Linux or `tracert` on Windows) is a diagnostic tool that maps the entire route (every router hop) taken by packets to reach a destination host.

> **Try it yourself:** Run `traceroute google.com` in your terminal (macOS/Linux), or `tracert google.com` on Windows. You will see live output listing every router hop along the path, with three RTT measurements per hop, exactly like the example output we examine below.

### The Clever Mechanics of Traceroute

Traceroute does not rely on routers voluntarily reporting their paths. Instead, it cleverly manipulates the **TTL field** in successive ICMP (or UDP) probe requests to discover routers one by one.

```mermaid
flowchart TD
    subgraph Step1 ["Step 1: Probe with TTL = 1"]
        direction LR
        S1["<b>My Machine</b><br>10.0.0.1"] -- "1. Request (TTL=1)" --> R1["<b>Home Router</b><br>10.0.0.4<br>(TTL drops 1 -> 0)" ]
        R1 -- "2. ICMP Time Exceeded (Source: 10.0.0.4)" --> S1
    end

    subgraph Step2 ["Step 2: Probe with TTL = 2"]
        direction LR
        S2["<b>My Machine</b><br>10.0.0.1"] -- "1. Request (TTL=2)" --> H2["<b>Home Router</b><br>10.0.0.4<br>(TTL: 2 -> 1)" ]
        H2 --> R2["<b>Router 20.0.0.1</b><br>(TTL drops 1 -> 0)" ]
        R2 -- "2. ICMP Time Exceeded (Source: 20.0.0.1)" --> S2
    end

    subgraph Step3 ["Step 3: Probe with TTL = 3"]
        direction LR
        S3["<b>My Machine</b><br>10.0.0.1"] -- "1. Request (TTL=3)" --> H3["<b>Home Router</b><br>10.0.0.4" ]
        H3 --> R3_1["<b>Router 20.0.0.1</b><br>(TTL: 2 -> 1)" ]
        R3_1 --> R3_2["<b>Router 30.0.0.1</b><br>(TTL drops 1 -> 0)" ]
        R3_2 -- "2. ICMP Time Exceeded (Source: 30.0.0.1)" --> S3
    end

    subgraph Step4 ["Final Step: Probe reaches Destination (TTL = N)"]
        direction LR
        S4["<b>My Machine</b><br>10.0.0.1"] -- "1. Request (TTL=N)" --> H4["<b>Intermediate Routers</b><br>(Passed successfully)" ]
        H4 --> D4["<b>google.com</b><br>60.0.0.1" ]
        D4 -- "2. ICMP Echo Reply (Route Complete!)" --> S4
    end

    Step1 --> Step2 --> Step3 --> Step4

    style Step1 fill:#18181b,stroke:#3f3f46,stroke-width:2px,color:#fff
    style S1 fill:#78350f,stroke:#fbbf24,color:#fff
    style R1 fill:#991b1b,stroke:#f87171,stroke-width:2px,color:#fff

    style Step2 fill:#18181b,stroke:#3f3f46,stroke-width:2px,color:#fff
    style S2 fill:#78350f,stroke:#fbbf24,color:#fff
    style H2 fill:#334155,stroke:#94a3b8,color:#fff
    style R2 fill:#991b1b,stroke:#f87171,stroke-width:2px,color:#fff

    style Step3 fill:#18181b,stroke:#3f3f46,stroke-width:2px,color:#fff
    style S3 fill:#78350f,stroke:#fbbf24,color:#fff
    style H3 fill:#334155,stroke:#94a3b8,color:#fff
    style R3_1 fill:#581c87,stroke:#c084fc,color:#fff
    style R3_2 fill:#991b1b,stroke:#f87171,stroke-width:2px,color:#fff

    style Step4 fill:#18181b,stroke:#3f3f46,stroke-width:2px,color:#fff
    style S4 fill:#78350f,stroke:#fbbf24,color:#fff
    style H4 fill:#581c87,stroke:#c084fc,color:#fff
    style D4 fill:#14532d,stroke:#86efac,stroke-width:2px,color:#fff
```

### Step-by-Step Traceroute Execution

1. **Hop 1 (TTL = 1)**: Traceroute sends a packet with `TTL = 1`. The first router (Home Router `10.0.0.4`) receives it, decrements TTL to 0, drops the packet, and returns an `ICMP Time Exceeded` message. Traceroute reads the source IP of the error message (`10.0.0.4`) and records it as Hop 1.
2. **Hop 2 (TTL = 2)**: Traceroute sends a packet with `TTL = 2`. The packet passes the home router (TTL becomes 1) and reaches the second router (`20.0.0.1`), where TTL decrements to 0. The second router drops the packet and returns `ICMP Time Exceeded`. Traceroute records `20.0.0.1` as Hop 2.
3. **Incremental Hops (TTL = 3, 4, ...)**: Traceroute increments TTL by 1 on each step, peeling back intermediate routers sequentially like layers of an onion.
4. **Reaching the Destination**: Eventually, TTL is large enough for the packet to reach the destination server (`60.0.0.1`). The target server does not expire the TTL, but responds with an **ICMP Echo Reply** (or Port Unreachable message if UDP probes are used). Traceroute recognizes the final response, logs the destination, and finishes execution.

---

## Deep Dive: Explaining `* * *` (Star Star Star) in Traceroute Output

When running `traceroute`, you will often see lines where router IP addresses are replaced by asterisks (`* * *`):

```text
traceroute to google.com (142.250.190.46), 64 hops max, 52 byte packets
 1  192.168.1.1 (192.168.1.1)  2.412 ms  1.854 ms  1.611 ms
 2  10.240.0.1 (10.240.0.1)  11.201 ms  10.844 ms  12.110 ms
 3  * * *
 4  142.250.238.15 (142.250.238.15)  18.450 ms  17.912 ms  18.102 ms
 5  142.250.190.46 (142.250.190.46)  19.120 ms  18.733 ms  18.910 ms
```

### Why Are There Three Asterisks?

By default, traceroute sends **3 probe packets** for each TTL value to calculate latency accurately across multiple attempts. 

Each asterisk (`*`) represents **one probe packet that timed out** without receiving a corresponding ICMP response. Therefore, `* * *` indicates that all three probe packets for that specific hop failed to receive an ICMP reply within the timeout period.

```mermaid
flowchart TD
    subgraph Laptop ["Traceroute Probe (TTL = 3)"]
        Send["<b>My Machine</b><br>10.0.0.1"]
    end

    subgraph Hop ["Hop 3 Router (30.0.0.1)"]
        R3["<b>ISP Router 3</b><br>Decrements TTL to 0 & Drops Packet"]
        Decision{"Is ICMP Enabled / Unfiltered?"}
    end

    subgraph Res1 ["Path A: ICMP Allowed"]
        ReturnOK["<b>Returns ICMP Time Exceeded</b><br>Source IP: 30.0.0.1"]
        LogOK["Traceroute records Hop 3: 30.0.0.1"]
    end

    subgraph Res2 ["Path B: ICMP Disabled / Firewalled"]
        ReturnDrop["<b>Silent Drop (No Response)</b><br>ICMP Blocked or Rate-Limited"]
        LogStar["Traceroute prints <b>* * *</b> (Timeout)"]
    end

    Send --> R3 --> Decision
    Decision -- "Yes" --> ReturnOK --> LogOK
    Decision -- "No" --> ReturnDrop --> LogStar

    style Laptop fill:#451a03,stroke:#f97316,color:#fff
    style Send fill:#78350f,stroke:#fbbf24,color:#fff

    style Hop fill:#1e293b,stroke:#64748b,color:#fff
    style R3 fill:#334155,stroke:#94a3b8,color:#fff
    style Decision fill:#1e1b4b,stroke:#6366f1,color:#fff

    style Res1 fill:#064e3b,stroke:#10b981,color:#fff
    style ReturnOK fill:#047857,stroke:#34d399,color:#fff
    style LogOK fill:#065f46,stroke:#6ee7b7,color:#fff

    style Res2 fill:#7f1d1d,stroke:#ef4444,color:#fff
    style ReturnDrop fill:#991b1b,stroke:#f87171,color:#fff
    style LogStar fill:#b91c1c,stroke:#fca5a5,color:#fff
```

### Primary Causes of `* * *` in Traceroute

1. **ICMP Responses Disabled on the Router**: Many ISP core routers are configured to process and forward data packets normally, but explicitly disable ICMP error message generation. The router decrements TTL to 0 and drops the probe packet, but sends nothing back. Because no ICMP response is generated, traceroute cannot discover the router's IP address.
2. **Firewall and Security Filtering**: Security appliances, enterprise firewalls, and cloud network security groups frequently block inbound ICMP traffic or drop TTL-expired ICMP responses to prevent external network mapping and topology discovery.
3. **ICMP Rate Limiting**: High-performance core routers prioritize transit user data and rate-limit low-priority ICMP error responses. If a router receives too many TTL-expired packets, it silently drops subsequent probes without replying.
4. **Asymmetric Routing and Packet Loss**: Return paths on the internet are not always symmetric. The ICMP response packet generated by a router may take a different path back to your laptop and get dropped due to network congestion or link degradation.


> Seeing `* * *` on an intermediate hop does **not** necessarily mean network failure! If subsequent hops (e.g., Hop 4 and Hop 5 above) respond successfully, data is flowing through Hop 3 normally. The router at Hop 3 is simply refusing to respond to ICMP probes.

---

## Summary and Key Takeaways

* **ICMP Protocol (Layer 3)**  
  Communicates diagnostic status, control notifications, and network error reports across IP networks without using port numbers.

* **Ping Utility**  
  Sends ICMP Echo Requests to check if a remote host is online and measures Round Trip Time (RTT) latency.

* **Time-To-Live (TTL)**  
  Decrements by 1 at each router hop to prevent infinite routing loops; triggers an `ICMP Time Exceeded` message when reaching 0.

* **Traceroute Utility**  
  Incrementally increases TTL starting from 1 to map every router hop along the network path to a destination host.

* **Asterisks (`* * *`) Output**  
  Indicates that intermediate routers or security firewalls intentionally blocked, rate-limited, or dropped ICMP responses for probes at that specific hop.

In the next post, we will explore Transport Layer concepts, examining how TCP and UDP establish reliable and fast socket connections across applications!
