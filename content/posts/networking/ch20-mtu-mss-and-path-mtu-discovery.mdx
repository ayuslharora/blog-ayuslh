...
Say your network's MTU is 1500 bytes, but the original IP packet you need to send is **1600 bytes**: a 20-byte IP header plus a 1580-byte TCP segment. Since 1600 exceeds the 1500 MTU, the TCP/IP stack **fragments** the packet into two independent IP packets.
...
> **Teardrop attack:** a classic real-world example of fragmentation's security risk. The attacker sends deliberately malformed fragments with **overlapping offsets**, so when the receiving stack tries to reassemble them, the byte ranges don't line up cleanly. Older TCP/IP implementations (Windows 95/NT, early Linux kernels) didn't handle this overlap correctly and would **crash or hang** while trying to reassemble the packet, a pure denial-of-service exploit. Modern stacks patch against it, but it's a good illustration of why reassembly logic is a meaningful attack surface, not just a performance cost.
