...
Say `ayuslh.in`'s DNS lives on Netlify, but `api.ayuslh.in`'s records live on Route 53 instead. Netlify's zone for `ayuslh.in` doesn't have an A record for `api.ayuslh.in`, it has an NS record delegating that subdomain to Route 53's name servers:
