...
```
example.com    MX    10    smtp1.securedserver.net.
example.com    MX    20    smtp2.securedserver.net.
```

Here's the full flow when `bob@gmail.com` sends mail to `hello@example.com`:

```mermaid
sequenceDiagram
    autonumber
    participant Sender as Gmail's mail server
    participant DNS
    participant SMTP1 as smtp1.securedserver.net<br/>(priority 10)

    Sender->>DNS: MX records for example.com?
    DNS-->>Sender: smtp1 (priority 10), smtp2 (priority 20)
    Note over Sender: Lowest priority wins, try smtp1 first
    Sender->>DNS: A record for smtp1.securedserver.net?
    DNS-->>Sender: 11.22.33.44
    Sender->>SMTP1: SMTP connection on port 25, deliver mail
    Note over SMTP1: Syncs into hello@example.com's<br/>inbox via IMAP/POP3
```

`example.com` itself never appears as a mail server anywhere in that flow, it's purely an identity. The actual delivery happens between two SMTP servers, entirely found through MX records and the A record lookup that follows. If `smtp1` is down, Gmail's mail server falls back to `smtp2`, the next-lowest priority. Two MX records at the *same* priority is also valid, and gets used for load balancing, senders pick between them effectively at random.
