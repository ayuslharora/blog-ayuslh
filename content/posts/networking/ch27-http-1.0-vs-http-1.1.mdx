...
![How HTTP/1.0 works: a separate TCP connection, established and closed, for each of index.html, image.jpg, and about.html](/images/ch27-http10-connection-per-request.png)
...
sequenceDiagram
    ...
    Note over Server: index.html is large,<br/>still processing
    Note over Server: image.jpg and about.html<br/>are ready, but blocked
    Server-->>Client: index.html response
    Server-->>Client: image.jpg response
    Server-->>Client: about.html response
```
