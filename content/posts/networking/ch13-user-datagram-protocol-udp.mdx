...
flowchart TD
    subgraph Phase1 ["Phase 1: TCP 3-Way Handshake (3 Trips)"]
        direction LR
        C1["<b>Client Host</b>"] -- "1. SYN" --> D1["<b>DNS Server (Port 53)</b>"]
        D1 -- "2. SYN-ACK" --> C1
        C1 -- "3. ACK" --> D1
    end
    ...
    subgraph Phase2 ["Phase 2: DNS Query & Data Exchange (4 Trips)"]
        direction LR
        C2["<b>Client Host</b>"] -- "4. DNS Query: example.com" --> D2["<b>DNS Server (Port 53)</b>"]
        D2 -- "5. ACK (Query Received)" --> C2
        D2 -- "6. Response: 10.0.0.1" --> C2
        C2 -- "7. ACK (Response Received)" --> D2
    end

    subgraph Phase3 ["Phase 3: TCP 4-Way Connection Teardown (4 Trips)"]
        direction LR
        C3["<b>Client Host</b>"] -- "8. FIN (Close Connection)" --> D3["<b>DNS Server (Port 53)</b>"]
        D3 -- "9. ACK" --> C3
        D3 -- "10. FIN" --> C3
        C3 -- "11. ACK" --> D3
    end
    ...
    style C1 fill:#78350f,stroke:#fbbf24,color:#fff
    style D1 fill:#581c87,stroke:#c084fc,color:#fff

    style Phase2 fill:#18181b,stroke:#3f3f46,stroke-width:2px,color:#fff
    style C2 fill:#78350f,stroke:#fbbf24,color:#fff
    style D2 fill:#581c87,stroke:#c084fc,color:#fff

    style Phase3 fill:#18181b,stroke:#3f3f46,stroke-width:2px,color:#fff
    style C3 fill:#78350f,stroke:#fbbf24,color:#fff
    style D3 fill:#581c87,stroke:#c084fc,color:#fff
```
...
```mermaid
flowchart TD
    subgraph DirectUDP ["UDP Query & Response (No Handshake or Teardown)"]
        direction LR
        C_UDP["<b>Client Host</b>"] -- "1. Query: IP of example.com? (Query ID: 1)" --> D_UDP["<b>DNS Server (Port 53)</b>"]
        D_UDP -- "2. Response: 10.0.0.1 (Query ID: 1)" --> C_UDP
    end

    style DirectUDP fill:#18181b,stroke:#3f3f46,stroke-width:2px,color:#fff
    style C_UDP fill:#78350f,stroke:#fbbf24,color:#fff
    style D_UDP fill:#14532d,stroke:#86efac,stroke-width:2px,color:#fff
```
