...
| **Reliability** | Unreliable (No Retransmissions) | Reliable (Guaranteed Delivery) |
| **State** | Stateless (Remembers Nothing) | Stateful (Tracks Sequences & Windows) |
| **Header Size** | 8 Bytes | 20 to 60 Bytes |
| **Speed & Overhead** | Ultra-Fast, Low Overhead | Slower, Higher Overhead |
| **Ordering** | Unordered (Packets can arrive out of order) | Strictly Ordered (Sequenced) |
...
sequenceDiagram
    ...
    Note over Client,DNS: --- TCP 3-Way Handshake ---
    Client->>DNS: 1. SYN
    DNS-->>Client: 2. SYN-ACK
    Client->>DNS: 3. ACK

    Note over Client,DNS: --- DNS Query & Data Exchange ---
    Client->>DNS: 4. DNS Query ("What is IP of example.com?")
    DNS-->>Client: 5. ACK (Query Received)
    DNS-->>Client: 6. DNS Response ("IP is 10.0.0.1")
    Client->>DNS: 7. ACK (Response Received)

    Note over Client,DNS: --- TCP 4-Way Connection Teardown ---
    Client->>DNS: 8. FIN (Close Connection)
    DNS-->>Client: 9. ACK
    DNS-->>Client: 10. FIN
    Client->>DNS: 11. ACK

    Note over Client,DNS: Total: 11 Network Hops for 1 DNS Lookup!
```
...
sequenceDiagram
    ...
    Client->>DNS: 1. UDP Datagram: DNS Query (Query ID: 1, "IP of example.com?")
    DNS-->>Client: 2. UDP Datagram: DNS Response (Query ID: 1, "IP is 10.0.0.1")

    Note over Client,DNS: Total: Only 2 Network Trips!
```
