...
xychart-beta
    ...
    line [128, 128, 128, 128, 128, 128, 128, 128, 128]
```

*Orange: cwnd. Violet: `ssthresh` (128).* Everything up to `RTT 7`, where the orange curve meets the flat violet `ssthresh` line, is Slow Start's exponential doubling; the crawl from 128 to 129 that takes an entire additional round trip is Congestion Avoidance already taking over.
...
```mermaid
xychart-beta
    title "cwnd after Triple Duplicate ACK / ECN: a shallow V"
    x-axis ["RTT 0", "RTT 1", "RTT 2", "RTT 3", "RTT 4", "RTT 5", "RTT 6", "RTT 7", "Congestion (800)", "New ssthresh (400)", "RTT +1", "RTT +2"]
    y-axis "cwnd (bytes)" 0 --> 800
    line [1, 2, 4, 8, 16, 32, 64, 128, 800, 400, 420, 440]
    line [128, 128, 128, 128, 128, 128, 128, 128, 400, 400, 400, 400]
```

*Orange: cwnd. Violet: `ssthresh`, stepping from its original 128 up to the recalculated 400 the moment congestion hits.* Notice the shape: cwnd only drops halfway, to 400, and immediately resumes climbing from there with Congestion Avoidance's gentle linear growth. It never touches Slow Start again.
...
```mermaid
xychart-beta
    title "cwnd after Retransmission Timeout: a deep U"
    x-axis ["RTT 0", "RTT 1", "RTT 2", "RTT 3", "RTT 4", "RTT 5", "RTT 6", "RTT 7", "Timeout (800->1)", "RTT +1", "RTT +2", "RTT +3", "RTT +4", "RTT +5", "RTT +6", "RTT +7", "New ssthresh (400)"]
    y-axis "cwnd (bytes)" 0 --> 800
    line [1, 2, 4, 8, 16, 32, 64, 128, 1, 2, 4, 8, 16, 32, 64, 128, 400]
    line [128, 128, 128, 128, 128, 128, 128, 128, 400, 400, 400, 400, 400, 400, 400, 400, 400]
```

*Orange: cwnd. Violet: `ssthresh`, stepping from 128 to 400 the instant the timeout crashes cwnd back to 1.* Compare the shape to the triple-duplicate-ACK/ECN case above: instead of a shallow dip to half, cwnd crashes all the way down to 1 MSS and has to climb the entire exponential Slow Start curve over again, this time stopping at the new ssthresh of 400 (instead of the original 128) before handing off to Congestion Avoidance.
