...
That choice isn't unconditional, though. UDP has a practical cap on how much data fits in a single datagram, traditionally 512 bytes for DNS, larger today with extensions, but still a hard ceiling. If a DNS response would exceed that, the server sends back what fits and sets a **truncated (TC) flag** in the response instead of just silently cutting it off. Seeing that flag, the resolver **retries the exact same query over TCP**, which has no such per-message size limit, and gets the complete answer back that way. This shows up in practice with unusually large responses, DNSSEC-signed answers, records with many entries, that kind of thing, while the overwhelming majority of ordinary DNS queries never come close to that ceiling and stay on UDP the whole way.
...
Given millions of DNS queries happen every second, it's fair to wonder whether just 13 addresses handling all of that traffic would buckle under the load. Two things keep that from happening. First, most queries never actually reach a root server at all: the referral a resolver gets back (which name servers handle `.com`, `.in`, and so on) rarely changes, so resolvers cache it aggressively, often for a day or two, meaning the root tier only gets hit on a cache miss, a small fraction of total DNS traffic. Second, each of those 13 addresses isn't one physical machine sitting in one location. They're served over **anycast**: the same IP address is announced from hundreds of physical server instances scattered around the world, and BGP routing sends each query to whichever instance is topologically nearest to the resolver that sent it. A "single" root server address might really be over a hundred machines across dozens of countries, all sharing the load and none of them a single point of failure. Combine small, cheap-to-answer responses (just NS and glue records) with caching and global anycast distribution, and 13 addresses turns out to be plenty.
