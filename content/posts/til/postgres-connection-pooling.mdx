...
As context switching overhead mounts, query latency spikes across p50, p90, and p99 metrics.

> Run the same query many times and sort the latencies from fastest to slowest. **p50** is the middle value: half your queries were faster, half slower, so it represents the typical case. **p90** is the value below which 90% of queries fall, the first hint of the slow tail. **p99** is the value below which 99% of queries fall, essentially your worst-case experience. An average can hide a handful of very slow queries by blending them with many fast ones; percentiles expose that tail directly, which is exactly what causes connections to be held open longer and pools to run dry under load.

In severe cases, extreme load causes OS kernel lockups or triggers process kills.
